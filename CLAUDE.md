...
- **ตรวจ `packLayout` ฝั่ง std (ไม่ใช้พาเลท) แบบละเอียด** — รอบนี้ตรวจเฉพาะ path พาเลท (ที่ AOF สังเกต)
  ยังไม่ได้ยิง invariant harness ชุดเดียวกันกับ path ปกติ **ควรทำต่อ** (harness เขียนไว้แล้ว ปรับ input
  เป็น usePallet:false ก็ใช้ได้เลย)
