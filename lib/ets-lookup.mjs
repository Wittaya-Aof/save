...
export async function searchVesselActualDate(page, vesselName, mode = 'sea', voyage = null) {
  ...
  try {
    ...
    const result = await page.evaluate(({ name, voy }) => {
      ...
      for (const t of tables) {
        ...
        if (voy && voyageIdx >= 0) {
          ...
          if (!picked) return { voyageMismatch: true, totalRows: rowsWithCells.length };
        } else {
        ...
        if (actualIdx < 0) return { noActualColumn: true, totalRows: rowsWithCells.length };
    ...
    if (result.voyageMismatch) return { eta: null, status: 'not_found', reason: 'ไม่พบเที่ยว (voyage) ที่ตรงในผลลัพธ์', totalVoyagesFound: result.totalRows };
