    ...
    const result = await page.evaluate(({ name, voy }) => {
      ...
      for (const t of tables) {
        ...
        if (voy && voyageIdx >= 0) {
          ...
          const want = nv(voy);
          // ตรงเป๊ะก่อนเสมอ
          picked = rowsWithCells.find(x => nv(x.cells[voyageIdx]) === want);
          // ถ้าไม่เจอ ยอมให้ตารางมีส่วนต่อท้ายเพิ่ม (สายเรือบางรายเติมรหัสท้าย เช่น B/L "0XSP3S"
          // แต่ ETS แสดง "0XSP3S1NC") — ต้องเป็น prefix เท่านั้น และ voyage ที่ค้นต้องยาวพอ
          // ห้ามใช้ substring หลวมๆ: "76S" กับ "76N" ต่างกันที่ทิศ (South/North) = คนละเที่ยวจริง
          if (!picked && want.length >= 4) {
            const cand = rowsWithCells.filter(x => nv(x.cells[voyageIdx]).startsWith(want));
            if (cand.length === 1) picked = cand[0]; // กำกวมเมื่อเจอหลายแถว → ไม่เดา
          }
