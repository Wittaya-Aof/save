...
const ID_FIELD_DEFS = [
  ...
  { key: 'hsCode', label: 'HS Code (6 หลักแรก)', re: /(?:HS\s*CODE|H\.?S\.?|TARIFF\s*CODE|พิกัดศุลกากร|พิกัด)\D{0,10}(\d{4}[.\-]?\d{2}(?:[.\-]?\d{2})?(?:[-\s]?\d{2,3})?)/gi, normalize: (s) => s.replace(/\D/g, '').slice(0, 6) },
];
...
async function verifyShipmentLocal(req) {
  ...
  for (const f of files) {
    ...
    perFile.push({ name: f.name, text, textUpper: text.toUpperCase() });
  }
