...
คุณคือผู้ตรวจสอบเอกสาร shipment ของบริษัทนำเข้า-ส่งออกสินค้าความงามระหว่างไทยกับจีน/เกาหลี
ทำหน้าที่แทนที่ทีมงานที่ตรวจสอบเอกสารด้วยมือทีละใบ — งานของคุณคืออ่านเอกสารที่แนบมาทั้งหมด
(Commercial Invoice, Packing List, Bill of Lading, Form E/D/AK, ใบขนสินค้า ฯลฯ) แล้วตรวจไขว้
(cross-check) ระหว่างเอกสาร ตามหลักการเดียวกับที่ผู้ตรวจสอบมืออาชีพทำ — กฎเฉพาะของนำเข้า/
ส่งออกอยู่ในหัวข้อต่อท้ายด้านล่าง
...
// ยังไม่เคยผ่านการใช้งานจริง/สะสม edge case เหมือน IMPORT_ADDENDUM ข้างบน (ที่มาจากประสบการณ์ตรวจ
// เอกสารนำเข้าจริงมานาน) — เป็นชุดกฎเริ่มต้นตามหลักการทั่วไปของใบขนขาออกไทย ควรทบทวน/เพิ่มเติมเมื่อ
// เจอ edge case จริงจากการใช้งาน export mode ครั้งแรกๆ
const EXPORT_ADDENDUM = `
## กฎเฉพาะใบขนสินค้าขาออก (กศก.101/1) เพิ่มเติม
- สินค้าส่งออกจากไทยส่วนใหญ่ไม่มีอากรขาออกและ VAT คิดในอัตรา 0% (zero-rated) — ไม่ต้องคำนวณ
  ภาษีแบบฝั่งนำเข้า ให้ส่ง taxCheck เป็น null เสมอ ยกเว้นเอกสารระบุชัดเจนว่ามีอากรขาออกจริง
  สำหรับสินค้ารายการนั้น (พบน้อยมาก)
- ผู้ส่งออก (Box 1 ใน Form E/D/AK) ต้องเป็นนิติบุคคลไทยที่ตรงกับผู้ขายใน Invoice — เลขประจำตัว
  ผู้เสียภาษีต้องตรงกับที่ระบุในใบขนขาออก
- ประเทศกำเนิดสินค้า (country of origin) ต้องเป็นไทยเสมอสำหรับ Form E/D/AK ที่ออกฝั่งส่งออก —
  ถ้าระบุประเทศอื่นถือเป็นข้อผิดพลาดที่ต้องแก้
- FOB ในใบขนขาออกต้องตรงกับ FOB ใน Commercial Invoice (ไม่ใช่ CIF) — ถ้า Invoice ระบุเป็น CIF
  ให้แยกค่าระวาง/ประกันภัยออกก่อนเทียบกับ FOB ในใบขน
- HS code ต้องตรงกัน 6 หลักแรกระหว่าง Form E/D/AK กับใบขนขาออก เช่นเดียวกับฝั่งนำเข้า
- ชื่อเรือ/เที่ยวบินใน B/L หรือ AWB คือแหล่งอ้างอิงที่ถูกต้องที่สุด ต้องตรงกับที่ระบุในใบขนขาออก
  และ Form E/D/AK (Box 3)
- ถ้า Invoice เป็น FOB → B/L มักเป็น Freight Collect / ถ้าเป็น CIF หรือ CFR → มักเป็น Freight
  Prepaid เช่นเดียวกับฝั่งนำเข้า ถือเป็นข้อสังเกตไม่ใช่ error เสมอไป`.trim();

function buildSystemPrompt(mode) {
  const isExport = mode === 'export';
  const addendum = isExport ? EXPORT_ADDENDUM : IMPORT_ADDENDUM;
  return SHARED_INSTRUCTIONS + '\n\n' + addendum +
    ...
    'และใส่บทสรุป + ข้อเสนอแนะโดยรวมไว้ใน summary — ถ้าคำนวณภาษีที่เกี่ยวข้องได้ ให้กรอก taxCheck ' +
    'ด้วยตัวเลขจริงที่คำนวณจากเอกสาร ถ้าคำนวณไม่ได้หรือไม่เกี่ยวข้อง' +
    (isExport ? ' (shipment ขาออกปกติไม่มีภาษีนำเข้าให้คำนวณ)' : '') +
    ' ให้ส่ง taxCheck เป็น null\n\n' +
    'นอกจากนี้ให้กรอก shipmentInfo ด้วยข้อมูลที่ดึงได้จาก B/L หรือ AWB: etd คือวันที่ "Shipped on ' +
    'Board" ใน B/L (หรือวันเที่ยวบินใน AWB) แปลงเป็นรูปแบบ YYYY-MM-DD เสมอ (ถ้าเป็นปี พ.ศ. ต้อง ' +
    'ลบ 543 ก่อน) — B/L คือแหล่งอ้างอิงที่ถูกต้องที่สุดตามที่ระบุไว้ข้างต้น ถ้าเป็น Draft B/L ที่ยัง ' +
    'ไม่มีวันที่ Shipped on Board (ช่องว่าง) ให้ส่ง etd เป็น null อย่าเดา/ประมาณวันที่เอง ถ้าเอกสารที่ ' +
    'แนบมาไม่มี B/L หรือ AWB เลย ให้ส่ง shipmentInfo ทั้งก้อนเป็น null';
}
    ...
    shipmentInfo: {
      anyOf: [
        { type: 'null' },
        {
          type: 'object',
          properties: {
            etd:           { type: ['string', 'null'], description: 'วันที่ Shipped on Board (B/L) หรือวันเที่ยวบิน (AWB) รูปแบบ YYYY-MM-DD — null ถ้าเป็น draft ที่ยังไม่มีวันที่จริง' },
            vessel:        { type: 'string' },
            blOrAwbNo:     { type: 'string' },
            portOfLoading: { type: 'string' },
          },
          required: ['etd', 'vessel', 'blOrAwbNo', 'portOfLoading'],
          additionalProperties: false,
        },
      ],
    },
  },
  required: ['status', 'sections', 'summary', 'taxCheck', 'shipmentInfo'],
  additionalProperties: false,
};
