...
async function callVerify({ mode, documentBlocks, excelText }) {
  ...
  const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY, timeout: 120000, maxRetries: 2 });
  ...
  content.push({ type: 'text', text: `ตรวจสอบเอกสาร shipment นี้ (${modeLabel}) ตามคำสั่งข้างต้น แล้วเรียก tool report_verification พร้อมผลลัพธ์` });
